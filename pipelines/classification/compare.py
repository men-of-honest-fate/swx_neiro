"""
Сравнение наборов признаков для классификации СПС.
...
def plot_heatmap(df: pd.DataFrame, phase: str, metric: str, out_path: Path):
    ...
    # Для log_loss и brier: меньше = лучше → cmap normal
    # Для accuracy и auc: больше = лучше → cmap reversed
    ...
    ax.set_title(f"{metric} ({phase_lbl})", fontsize=10)
    plt.tight_layout()
...
def plot_best_per_featureset(df: pd.DataFrame, phase: str, out_path: Path):
    ...
    ax.set_ylabel(f"Log Loss (лучшая модель)")
    phase_lbl = "CV" if phase == "cv" else "Test SC25"
    ax.set_title(f"Лучший Log Loss по набору признаков ({phase_lbl})")
...
def print_summary(df: pd.DataFrame, phase: str):
    ...
    phase_lbl = "CV" if phase == "cv" else "Test SC25"
    print(f"\nLog Loss ({phase_lbl}), меньше = лучше:")
...
def main():
    ...
    print("\nОбучение классификаторов по всем конфигурациям...")
    ...
    for phase in ("cv", "test"):
        ...
        plot_best_per_featureset(df, phase,
                                 PLOTS_DIR / f"best_{phase}.png")
