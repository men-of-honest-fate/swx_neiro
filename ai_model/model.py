...
def neiro_fit():
    ...
    table_in = [[row["Localization_x"], row["Localization_y"], row["Importance (Xray/Opt)"]] for index, row in
                table.iterrows()]
    table_out1 = [[row["Jmax1 (pfu)"]] for index, row in table.iterrows()]

    X_train, X_test, y_train, y_test = train_test_split(table_in, table_out1, test_size=0.33, random_state=42)
    models1 = [RandomForestRegressor(n_estimators=100, max_features='sqrt'),  # случайный лес
    ...
    X_train, X_test, y_train, y_test = train_test_split(table_in, table_out2, test_size=0.33, random_state=42)
    models2 = [RandomForestRegressor(n_estimators=100, max_features='sqrt'),  # случайный лес
